...
def load_bert_data(bert_file="bert_chapter_results.csv"):
...
def combine_clip_bert_scores(clip_chapters, bert_scores, time_tolerance=10):
    ...
    for clip_chapter in clip_chapters:
        ...
        # Find nearest BERT score within tolerance
        best_bert_score = 0
        ...
        for bert_entry in bert_scores:
            ...
            # Check if CLIP timestamp is within BERT segment or close to it
        ...
        combined_score = {
            ...
            'combined_score': (
                ...
                best_bert_score * 2  # Weight audio score higher
            )
        }
...
def filter_chapters(chapters, min_gap_seconds=90, max_chapters=8):
    """Filter chapters by removing those too close together"""
    # Sort by timestamp
    sorted_chapters = sorted(chapters, key=lambda x: x['timestamp_seconds'])
    
    filtered = []
    last_time = -min_gap_seconds  # Start with negative to include first chapter
    
    for chapter in sorted_chapters:
        current_time = chapter['timestamp_seconds']
        
        # Only add if it's far enough from the last chapter
...
def create_final_chapters():
    ...
    for i, chapter in enumerate(final_chapters):
        ...
        youtube_chapters.append({
            "name": title,
            "start": seconds_to_mmss(chapter['timestamp_seconds'])
        })
    
    output = {
        "chapters": youtube_chapters,
        "total_chapters": len(youtube_chapters),
        "methodology": "Combined CLIP visual + BERT audio scores"
    }
...
if __name__ == "__main__":
    ...
    for chapter in final_chapters:
        print(f'{{ "name": "{chapter["name"]}", "start": {chapter["start"]} }},')